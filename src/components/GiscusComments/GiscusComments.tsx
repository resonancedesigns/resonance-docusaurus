 ...
 */
const GiscusCommentsContent: React.FC<{ config: any }> = ({ config }) => {
  ...
  if (
    ...
    return (
      <div style={{ padding: '20px', textAlign: 'center', color: '#666' }}>
        ...
        <p>
          Configure Giscus settings in <code>src/config/giscus-config.ts</code> to
          enable comments.
        </p>
        <p>
          <a
...
const GiscusComments: React.FC = () => {
  return (
    <FeatureComponent feature={Features.GiscusComments} configData={configData}>
      {(config) => <GiscusCommentsContent config={config} />}
    </FeatureComponent>
  );
};
